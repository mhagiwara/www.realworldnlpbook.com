...
Finally, you need to adjust the input vector size of your LSTM-RNN. By the way, this is not the biLSTM used by ELMo, but rather the LSTM you built to classify the sentence. Because we are using the ELMo embeddings as the input to this LSTM, you need to adjust the `input_size` parameter to `torch.nn.LSTM`: 
